...
const render = (title, employees) => {
  ...
  return renderMain(manager, html.join(""), title);
};
...
const renderMain = (manager, html, title) => {
  let template = fs.readFileSync(
    path.resolve(templatesDir, "main.html"),
    "utf8"
  );
  template = replacePlaceholders(template, "title", title);
